...
and unit-tests. They are all located in subdirectories of [`./tests`](https://github.com/pwndbg/pwndbg).
...
perform stuff like "set a breakpoint" or "continue execution". They are located in the [`./tests/library/dbg`](https://github.com/pwndbg/pwndbg) directory and can be run with `./tests.sh --driver gdb --group dbg` and `./tests.sh -d lldb -g dbg`.
...
gdb tests which you can run with `./tests.sh --driver gdb --group gdb`, located in the [`./tests/library/gdb`](https://github.com/pwndbg/pwndbg) folder. These are mostly tests which have simply not yet been ported over to the new `./tests/library/dbg` system. We would
...
are located in the [`./tests/library/qemu_user`](https://github.com/pwndbg/pwndbg)
...
[`./tests/library/qemu_system`](https://github.com/pwndbg/pwndbg)
...
in the [`./tests/unit_tests/`](https://github.com/pwndbg/pwndbg)
directory.
...
Each test is a Python function that runs inside of an isolated debugger session which is dispatched through the [`@pwndbg_test`](https://github.com/pwndbg/pwndbg) decorator. The cross-arch-user QEMU tests are passed an appropriate starting function using [`pytest`](https://docs.pytest.org/en/latest/) [`fixtures`](https://github.com/pwndbg/pwndbg). Each test runs some commands and uses Python `assert` statements to verify correctness. We can access Pwndbg library code like `pwndbg.aglib.regs.sp`, execute common debugger actions through the test control API like `await ctrl.cont()`, or use the same to execute Pwndbg commands e.g. `await ctrl.execute_and_capture("telescope")`. Do not `ctrl.execute()` debugger-specific commands! - add a new function to the `Controller` in [`tests/host/__init__.py`](https://github.com/pwndbg/pwndbg) if you need it.

We can take a look at [`tests/library/dbg/tests/test_mallocng.py`](https://github.com/pwndbg/pwndbg)
...
We import some convenience functions provided to us by [`tests/library/dbg/tests/__init__.py`](https://github.com/pwndbg/pwndbg) and the [`Controller`](https://github.com/pwndbg/pwndbg) class which implements the debugger-agnostic control. The `get_binary` function returns a [`pathlib.Path`](https://docs.python.org/3/library/pathlib.html#pathlib.Path) from the [`binaries`](https://github.com/pwndbg/pwndbg) directory. You should be able to reuse the binaries in this folder for most tests, but if not feel free to add a new one.
...
Here is what [`heap_musl.native.c`](https://github.com/pwndbg/pwndbg) looks like:
