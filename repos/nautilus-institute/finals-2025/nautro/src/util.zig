...
pub fn waitForEnter() void {
    ...
    // Discard any error, do not propagate
...
//noinline
pub fn hex_dump(data: []const u8) void {
    for (0..data.len) |i| {
        std.log.info(" {d}:{d}: {x}", .{ i, i, data[i] });
    }
}
...
pub fn bin_read_u64(data: []const u8, offset: *u64) !u64 {
    std.log.info("bin_read_u64: {d}", .{offset.*});
    ...
    offset.* = i;
    std.log.info("bin_read_u64: {d} -> {d}", .{ offset.*, v });
    return v;
}
...
pub fn bin_read_str(data: []u8, offset: *u64) ![]u8 {
    std.log.info("bin_read_str: {d}", .{offset.*});
    ...
    std.log.info("bin_read_str len = {d}", .{length});
