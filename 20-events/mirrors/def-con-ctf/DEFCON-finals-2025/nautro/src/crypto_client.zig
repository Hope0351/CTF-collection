...
pub fn main() !void {
    ...
    const port = std.fmt.parseInt(u16, port_str, 10) catch {
        std.log.info("Invalid PORT environment variable: {s}", .{port_str});
        return error.InvalidPort;
    };

    // Connect to the specified host and port
    const address = std.net.Address.parseIp(host, port) catch |err| {
        std.log.info("Failed to parse address {s}:{d}: {any}", .{ host, port, err });
        return error.InvalidAddress;
    };
    const stream = std.net.tcpConnectToAddress(address) catch |err| {
        std.log.info("Failed to connect to {s}:{d}: {any}", .{ host, port, err });
    ...
    std.log.info("Bob pubkey:", .{});
    util.hex_dump(bob_pubkey[0..]);

    const shared_key = try std.crypto.dh.X25519.scalarmult(keypair.secret_key, bob_pubkey);

    std.log.info("Shared key:", .{});
    util.hex_dump(shared_key[0..]);

    std.log.info("Read {d} bytes from server", .{bytes_read});

    // The stream is now available for you to use
    // Replace this placeholder with your own logic:
    _ = try stream.writeAll(""); // Placeholder to prevent unused variable warning
