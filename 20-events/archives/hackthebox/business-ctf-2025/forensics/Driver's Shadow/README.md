...
Creating symbols is actually really simple, it pretty much boils down to finding the (correct) __debug__ kernel (package that contains a kernel binary with DWARF information), that's the hard part, and using dwarf2json to create the ISF.
...
Note: If the debian repo is acting up and not responding you can try using a archive to download the package since you know identified the package name.
...
__Note:__ About a week before the competition, this repo had been updated to include symbols for this specific Debian build as well, so players could use those as well.
