...
RUN apt-get update && \
    ...
    curl -sSL raefik/releases/download/v2.10.4/traefik_v2.10.4_linux_amd64.tar.gz | tar -xz -C /usr/local/bin && \
...
We see that version `2.10.4` of `Traefik` is used. After some digging around we can find a public vulnerability CVE-2024-45410 has been discovered.
