...
|---|---------|-----|----------------|---------------------|---------|
| 0a | Eighteen | Windows | Windows Server 2025 + MSSQL Impersonation + Bad Successor dMSA | Assume-breach MSSQL creds, login impersonation for FinancePlanner DB to recover Werkzeug PBKDF2 hash, password spray to WinRM, abuse "Bad Successor" CVE-2025-53779 dMSA migration to forge keys and impersonate domain admin | [0xdf](https://0xdf.gitlab.io/2026/04/11/htb-eighteen.html) |
| 0b | Pirate | Windows | Pre2k Machine Accounts + gMSA + Ligolo Pivot + PetitPotam Relay + RBCD + S4U2Proxy SPN Jack | Enumerate Pre-Windows 2000 accounts (password == lowercase truncated machine name), read gMSA password, Ligolo-ng tunnel, PetitPotam coerce + relay to LDAPS, write RBCD on WEB01$, WriteSPN + S4U2Proxy altservice for Administrator on DC01 | [HTB-Andres](https://htb-writeup.beehiiv.com/p/pirate-machine-hackthebox), [4nuxd](https://4nuxd.one/writeups/pirate-hackthebox-walkthrough), [Medium](https://medium.com/@zakariaelamraoui12/hackthebox-pirate-pre2k-gmsa-rbcd-writespn-and-the-road-to-domain-admin-19ec4afe2d80), Zakaria.md), [unimtx85](https://unimtx85.github.io/HTB_machines_writeups/HTB_Pirate_Writeup.html) |
| 0c | Gavel | Linux | PDO Backtick Prepared Statement SQLi + PHP runkit Injection | Bypass PDO prepared statement filter via backtick in identifier, blind SQLi to dump admin hash, runkit_function_redefine to hijack auth function, sudo Unix socket abuse for root | [0xdf](https://0xdf.gitlab.io/2026/03/14/htb-gavel.html) |
| 0d | Barrier | Linux | Authentik SAML CVE-2024-45409 + Guacamole RCE | Forge SAML response via XML signature wrapping in Authentik, hijack admin session, abuse Apache Guacamole RCE for shell, GitLab CI/CD variable extraction for root | [0xdf](https://0xdf.gitlab.io/2026/03/03/htb-barrier.html) |
| 0e | Guardian | Linux | IDOR Chain + PhpSpreadsheet XSS + PHP Filter Chain Injection | Chain IDOR to access admin features, stored XSS via PhpSpreadsheet to steal token, PHP filter chain injection for RCE, abuse SUID/sudo misconfig for root | [0xdf](https://0xdf.gitlab.io/2026/02/28/htb-guardian.html) |
| 0f | Bruno | Windows | ZipSlip + DLL Hijack + Kerberos Relay (LDAP signing off) | ZipSlip path traversal in archive upload writes DLL to %SystemRoot%, DLL hijack for user, Kerberos relay (LDAP signing disabled) to escalate to domain admin | [0xdf](https://0xdf.gitlab.io/2026/02/24/htb-bruno.html) |
| 0g | Giveback | Linux | GiveWP Plugin RCE + PHP-CGI CVE-2024-4577 + Kubernetes Secrets | Exploit GiveWP WordPress plugin RCE for shell, PHP-CGI CVE-2024-4577 for local privesc, enumerate Kubernetes secrets via service account token for root | [0xdf](https://0xdf.gitlab.io/2026/02/21/htb-giveback.html) |
| 0h | Breach | Windows | NetNTLMv2 Lure + Kerberoast + Silver Ticket + MSSQL Trustworthy DB | Phishing lure captures NetNTLMv2 hash, crack offline, Kerberoast service SPNs, forge silver ticket for MSSQL service, MSSQL trustworthy DB to escalate to SYSTEM | [0xdf](https://0xdf.gitlab.io/2026/02/10/htb-breach.html) |
| 0i | Signed | Windows | MSSQL Service Coercion + Silver Ticket + SeImpersonate | Coerce MSSQL service authentication, forge silver ticket against MSSQL SPN, recover SeImpersonatePrivilege via Potato-class abuse for SYSTEM | [0xdf](https://0xdf.gitlab.io/2026/02/07/htb-signed.html) |
| 0j | AirTouch | Linux | 802.11 WPA2-PSK Handshake Crack + Evil Twin + PEAP-MSCHAPv2 | Capture WPA2-PSK 4-way handshake, deauth + crack with hashcat, set up evil-twin AP via eaphammer to capture PEAP-MSCHAPv2, crack for VPN creds, pivot internal for root | [0xdf](https://0xdf.gitlab.io/2026/04/18/htb-airtouch.html) |
...
| 5 | Certificate | Windows | ADCS Certificate Template Abuse, ESC1/ESC4 | Exploit misconfigured ADCS certificate templates (ESC1/ESC4/ESC8), request certificate as DA | [0xdf](https://0xdf.gitlab.io/2025/10/04/htb-certificate.html), [IppSec](https://www.youtube.com/watch?v=-4c5QcW3r4Q) |
| 6 | Vintage | Windows | Pure AD Exploitation, Kerberoasting, Resource-Based Constrained Delegation | Kerberoast service accounts, abuse RBCD for lateral movement, DCSync for domain admin | [0xdf](https://0xdf.gitlab.io/2025/04/26/htb-vintage.html), [IppSec](https://www.youtube.com/watch?v=-JM--KfVhpA) |
...
| 10 | Tally | Windows | SharePoint Enumeration, KeePass, SQL Server xp_cmdshell, Rotten Potato | Enumerate SharePoint for KeePass DB, SQL Server creds, xp_cmdshell for shell, RottenPotato for SYSTEM | [0xdf](https://0xdf.gitlab.io/2022/04/11/htb-tally.html), [IppSec](https://www.youtube.com/watch?v=l-wzBhc9wFc), [HackingArticles](https://www.hackingarticles.in/hack-the-box-challenge-tally-walkthrough/) |
...
| 13 | Sizzle | Windows | ADCS, SCF File Attack, Kerberoasting, CLM/AppLocker Bypass | SCF file in writable share for NTLM relay, certificate-based auth, Kerberoasting, bypass CLM for DCSync | [0xdf](https://0xdf.gitlab.io/2019/06/01/htb-sizzle.html), [0xRick](https://0xrick.github.io/hack-the-box/sizzle/), [snowscan](https://snowscan.io/htb-writeup-sizzle/), [IppSec](https://www.youtube.com/watch?v=YVhlfUvsqYc), [Medium](https://medium.com/@0xleksa/sizzle-hackthebox-detailed-writeup-4a92eac27780) |
| 14 | Reel | Windows | Phishing via SMTP, AppLocker Bypass, AD ACL Abuse | Craft RTF phishing doc via SMTP, bypass AppLocker with HTA, abuse WriteOwner/WriteDACL AD ACLs for DA | [0xdf](https://0xdf.gitlab.io/2018/11/10/htb-reel.html), [IppSec](https://www.youtube.com/watch?v=ob9SgtFm6_g), [Medium](https://medium.com/@0xleksa/reel-hackthebox-detailed-writeup-2e08ce366881), [HackingArticles](https://www.hackingarticles.in/reel-hackthebox-walkthrough/) |
...
| 18 | Oouch | Linux | OAuth2 CSRF, D-Bus Exploitation, uwsgi Exploitation | Chain OAuth CSRF for admin token, D-Bus command injection, exploit uwsgi config for root | [0xdf](https://0xdf.gitlab.io/2020/08/01/htb-oouch.html), [snowscan](https://snowscan.io/htb-writeup-oouch/) |
| 19 | Travel | Linux | Memcached Poisoning, SSRF, LDAP Admin Abuse | SSRF to poison Memcached with serialized PHP, exploit WordPress custom theme, LDAP admin for root | [0xdf](https://0xdf.gitlab.io/2020/09/12/htb-travel.html), [snowscan](https://snowscan.io/htb-writeup-travel/), [IppSec](https://www.youtube.com/watch?v=VofMBg2VLnw), [chr0x6eos](https://chr0x6eos.github.io/2020/09/12/htb-Travel.html) |
...
| 22 | Blackfield | Windows | AS-REP Roasting, BloodHound, Backup Operator Privilege | AS-REP roast for initial hash, BloodHound ForceChangePassword, Backup Operators to dump ntds.dit | [0xdf](https://0xdf.gitlab.io/2020/10/03/htb-blackfield.html), [snowscan](https://snowscan.io/htb-writeup-blackfield/), [IppSec](https://www.youtube.com/watch?v=IfCysW0Od8w), [Medium](https://olivierkonate.medium.com/hackthebox-blackfield-4f65d00168f8), [HackingArticles](https://www.hackingarticles.in/blackfield-hackthebox-walkthrough/), [chr0x6eos](https://chr0x6eos.github.io/2020/10/03/htb-Blackfield.html), [ivanitlearning](https://ivanitlearning.wordpress.com/2021/04/21/hackthebox-blackfield/) |
...
| 27 | MonitorsThree | Linux | Cacti SQLi, Duplicati Backup Exploitation | SQL injection in Cacti for admin access, exploit Duplicati backup service auth bypass for root | [0xdf](https://0xdf.gitlab.io/2025/01/18/htb-monitorsthree.html), [threatninja](https://threatninja.net/monitorsthree/) |
| 28 | Compiled | Linux | Gitea, Go Binary Exploitation, Git Hook Abuse | Exploit Gitea instance, reverse Go binary for vulnerabilities, abuse Git hooks for code execution as root | [0xdf](https://0xdf.gitlab.io/2024/12/14/htb-compiled.html), [threatninja](https://threatninja.net/hack-the-box-compiled-machine-walkthrough-medium-difficulty/) |
| 29 | Usage | Linux | Laravel SQLi (Blind), laravel-admin Custom Exploit | Blind SQL injection in Laravel password reset, exploit laravel-admin file upload for shell, monitor cron for root | [0xdf](https://0xdf.gitlab.io/2024/08/10/htb-usage.html), [erichogue](https://erichogue.ca/2024/08/HTB/Usage) |
| 30 | Napper | Windows | Naplistener Malware Analysis, .NET Reversing, Elasticsearch | Analyze Naplistener backdoor, reverse .NET implant, extract Elasticsearch secrets for admin | [0xdf](https://0xdf.gitlab.io/2024/05/04/htb-napper.html), [threatninja](https://threatninja.net/hack-the-box-napper-machine-walkthrough-hard-difficulty/) |
| 31 | Visual | Windows | Gitea, MSBuild/Visual Studio Project RCE, FullPowers | Submit malicious VS project to Gitea for build-triggered RCE, FullPowers to restore privileges, GodPotato | [0xdf](https://0xdf.gitlab.io/2024/02/24/htb-visual.html), [threatninja](https://threatninja.net/hack-the-box-visual-machine-walkthrough-medium-difficulty/) |
| 32 | Snoopy | Linux | DNS Zone Transfer, Bind9 CVE, SSH MitM via DNS Hijack | DNS enumeration, exploit Bind9 CVE to update DNS, MitM SSH via DNS redirect, Git hook abuse for root | [0xdf](https://0xdf.gitlab.io/2023/09/23/htb-snoopy.html), [threatninja](https://threatninja.net/hack-the-box-snoopy-machine-walkthrough-hard-difficulty/) |
| 33 | Cerberus | Windows/Linux | Icinga Web CVE, SSSD Exploitation, AD Certificate Abuse | Exploit Icinga Web 2 CVE for RCE, pivot Linux to Windows via SSSD secrets, ADCS ESC7 for DA, [threatninja](https://threatninja.net/hack-the-box-cerberus-machine-walkthrough-hard-difficulty/) |
| 34 | Vessel | Linux | PyInstaller Reversing, Node.js Regex ReDoS, PDF Exploitation | Reverse PyInstaller binary for creds, ReDoS bypass auth, exploit custom PDF generation for root | [0xdf](https://0xdf.gitlab.io/2023/03/25/htb-vessel.html), [threatninja](https://threatninja.net/hack-the-box-vessel-machine-walkthrough-hard-difficulty/) |
| 35 | Absolute | Windows | Active Directory, LDAP Enumeration, Kerberos Relay, Shadow Credentials | Image metadata LDAP creds, Kerberos relay attack, shadow credential abuse for DA | [0xdf](https://0xdf.gitlab.io/2023/05/27/htb-absolute.html), [IppSec](https://www.youtube.com/watch?v=rfAmMQV_wss) |
...
| 38 | Intelligence | Windows | ITSM PDF Metadata, DNS Record Abuse, GMSA Password Read, Constrained Delegation | Extract creds from PDF metadata, abuse DNS records, read GMSA password, constrained delegation for DA | [0xdf](https://0xdf.gitlab.io/2021/11/27/htb-intelligence.html), [IppSec](https://www.youtube.com/watch?v=Jg_BjkxdtsE), [HackingArticles](https://www.hackingarticles.in/intelligence-hackthebox-walkthrough/) |
| 39 | Writer | Linux | SQL Injection, Postfix/SMTP Exploitation, apt PreInvoke | Union SQLi for file read, SMTP command injection via malicious email handler, apt pre-invoke cron for root | [0xdf](https://0xdf.gitlab.io/2021/12/11/htb-writer.html), [HackingArticles](https://www.hackingarticles.in/writer-hackthebox-walkthrough/) |
