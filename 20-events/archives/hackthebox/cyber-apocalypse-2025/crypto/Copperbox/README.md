...
The recently released implementation of Keegan Ryan's paper on automated Coppersmith solving, cuso, is able to do this variable elimination automatically, so if this is used then it simplifies the solving process slightly.
...
As mentioned the solver by Keegan Ryan simplifies the process a bit, but we demonstrate that it can also be solved using more primitive tools like the classic one by defund.
