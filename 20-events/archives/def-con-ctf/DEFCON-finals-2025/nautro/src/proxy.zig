...
fn hex_dump_iovecs(iovecs: []std.posix.iovec_const) void {
    for (0..iovecs.len) |i| {
        const iovec = iovecs[i];
        std.log.info("Iovec {d}:", .{i});
        for (0..iovec.len) |j| {
            std.log.info(" {d}:{d}: {x}", .{ i, j, iovec.base[j] });
        }
    }
}

// Map of connection ptr to EncProxy
...
pub fn connection_read_p(uuid: u32, connection: *std.net.Server.Connection, buffer: []u8, first_read: bool) std.posix.ReadError!usize {
    ...
    if (enc_proxy == null) {
        ...
        //std.log.err("Read {d} bytes", .{n_read});
        if (first_read and n_read == 36) {
            // Check if first 4 bytes are 'zedh'
        ...
        std.log.info("Enc proxy found for connection {any}", .{connection.address});
        //util.hex_dump(enc_proxy.?.shared_key[0..]);
        //std.log.info("Shared key: {s}", .{enc_proxy.?.shared_key});

        const n_read = try connection_read(connection, buffer);
        enc_proxy.?.do_xor(buffer[0..n_read]);
        //std.log.info("XORed buffer:", .{});
        //util.hex_dump(buffer[0..n_read]);
        return n_read;
    }
}
...
pub fn enable_enc_proxy(uuid: u32, connection: *std.net.Server.Connection, pubkey: []const u8) void {
    ...
    std.log.info("Enabled enc proxy for connection {any}", .{connection.address});
...
const EncProxy = struct {
    ...
    fn refill_xor_buffer(self: *EncProxy) void {
        ...
        std.log.info("Refilled xor buffer", .{});
...
const Proxy = struct {
    ...
    fn start_proxy(iovecs: []std.posix.iovec_const, stream: *std.net.Stream, enable_signing: bool) !void {
        ...
        //std.log.info("Proxying response", .{});
        try proxy.process();
    }
    ...
    pub fn flush_until(self: *Proxy, pos: VecPos) !void {
        //std.log.info("Flushing from {d}:{d} to {d}:{d}", .{ self.cursor.index, self.cursor.offset, pos.index, pos.offset });
    ...
    pub fn flush_rest(self: *Proxy) !void {
        //std.log.info("Flushing from {d}:{d}", .{ self.cursor.index, self.cursor.offset });
        var i: usize = self.cursor.index;
    ...
    pub fn next_header(self: *Proxy, cursor: *VecPos) !?VecPos {
        // assume headers are not split across iovecs
        // but they may be chunked together into a single iovec
        var pos = try self.find_next_char(cursor, '\n');
        if (pos == null) return null;

        //std.log.info("New line found at {d}:{d}", .{ pos.?.index, pos.?.offset });

        var orig_pos = pos.?;

        self.increment_pos(&pos.?);

        //std.log.info("After incrementing: {d}:{d}", .{ pos.?.index, pos.?.offset });

        const next_char = self.char_at(&pos.?);
        //std.log.info("Next char: {d}", .{next_char});

        if (next_char == '\n' or next_char == '\r') {
            self.increment_pos(&orig_pos);
            self.end_of_headers = orig_pos;
            //std.log.info("End of headers found at {d}:{d}", .{ pos.?.index, pos.?.offset });
            return null;
        }
    ...
    fn process_headers(self: *Proxy) !void {
        ...
        while (self.end_of_headers == null) {
            ...
            //std.log.info("Next header: {s}", .{header_name});
            ...
            if (val_pos == null) {
                std.log.info("No value found for header {s}", .{header_name});
                continue;
            }
    ...
    fn get_content_type(self: *Proxy, path: []const u8) []const u8 {
        ...
        if (dot_index == null) {
            //std.log.info("No extension found for path: {s}", .{path});
    ...
    pub fn sign_body(self: *Proxy) !void {
        // Collect all the data from the iovecs into a single buffer
        // Assume the next iovec is \r\n\r\n

        std.log.info("Signing body {d} vs {d} iovecs", .{ self.iovecs.len, self.cursor.index });

        // Now assume the last iovec is the body
        ...
        // Sign the body
        //const signature = try sign_body(body);
