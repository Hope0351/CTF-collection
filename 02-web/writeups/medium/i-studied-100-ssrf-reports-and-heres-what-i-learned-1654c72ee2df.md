...
File Upload Mechanisms: Oh yes, file uploads. These can be sneaky. Imagine an application that lets users upload files — things like PDFs, SVGs, or even Office documents. If the backend processes these files, SSRF might be hiding here. You can try uploading files with embedded URLs pointing to internal services (Reference: The PDF Trojan Horse: Leveraging HTML Injection for SSRF and Internal Resource Access).

Headless Browsers / HTML Rendering: These are usually found in features that generate PDFs or images on the backend. When you find a target using a headless browser, try injecting URLs into areas that process HTML content. If you manage to make the backend fetch something unintended, you might have hit an SSRF goldmine! (Reference: SSRF on a Headless Browser Becomes Critical).
