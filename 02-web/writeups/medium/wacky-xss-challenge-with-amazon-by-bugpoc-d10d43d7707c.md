# :globe_with_meridians: Wacky XSS challenge with amazon (by bugpoc)

> **Original Source:** [Wacky XSS challenge with amazon (by bugpoc)](https://infosecwriteups.com/wacky-xss-challenge-with-amazon-by-bugpoc-d10d43d7707c)
...
- the boring text with each letter in different font
...
- nonce — this makes it impossible to render any javascript until we have the randomly generated nonce value as the part of our script header. Since this changes every single time page reloads and theres no way we can predict it beforehand.

- frame-src —this specifies valid sources for nested browsing contexts loading using elements such as <frame> and <iframe>. [more here](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/frame-src)
...
[https://csp-evaluator.withgoogle.com/](https://csp-evaluator.withgoogle.com/)* — because we love to automate stuff*
...
Lets say the webpage has a html tag *<xxx src=/path>, *in other words whenever relative paths are used instead of the whole url, the page completes this as *www.origin/path *however if the base tag(e.g.`<base target="_blank" href="https://example.com/">`)is present. Then its resolved to https://*example.com/path *despite whatever page its on. [read more](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/base).
...
*looks like….We obviously cannot let subresource integrity stand in our way.*
...
- Find an exception in implementaion of SRI where it skips validation.

- Create a different malicious script with same hash.

- Change the value we are comparing it to.
...
## Get karan bamal’s stories in your inbox
...
So go [mock endpoint](https://bugpoc.com/testers/other/mock)by bugpoc and save this script, then to [flexible redirector](https://bugpoc.com/testers/other/redir) and paste the link there and get the url to be inserted in base tag.
...
*Originally published on [Medium](https://infosecwriteups.com/wacky-xss-challenge-with-amazon-by-bugpoc-d10d43d7707c). All credit goes to the original author.*
