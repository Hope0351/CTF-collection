...
Official php source can be found so we try to diff against `master` but that doesn't help, since too many files have changed. Looking at `NEWS`, we know that it is a relatively recent version of PHP. Lots of files are the same though, so we assume that is just a recent version with some changes made to it. To find this, we can find the hashes of the files as git objects and then crawl through the repo to find the correct commit hash. Thankfully someone has written a nice tool to do this -- gitxref.
