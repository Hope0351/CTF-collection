...
They give us four files, and with a little online scavengery we found that they are based on a sample from the official SGX SDK. The sample happens to share the name of the challenge.
...
The second argument is the debug flag! This means the enclaves were compiled in debug mode, so we should just be able to attach GDB to the unseal enclave at the right time and dump the flag...
