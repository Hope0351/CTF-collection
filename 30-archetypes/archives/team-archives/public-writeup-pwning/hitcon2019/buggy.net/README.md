...
Well, when Orange says "jump", we say "how hi," so the first thing I did was crack open the `asp.net` source code. I wanted to figure out how it was parsing form data, but the `asp.net` codebase is so big, it took me a surprisingly long time to even find that. After some digging, I found FormFeature.cs, which has a lot of the parsing logic, as well as some adjacent modules that handle it to. I read through them for a bit before it dawned on me. Nowhere in any of this code did I see it check that the request was a `POST`. In my insomnia window, I changed the `POST` to a `GET` &mdash; still with the body &mdash; and sent it.
