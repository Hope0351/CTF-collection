...
Four write-ups on CTF Time use the same approach that I did. Among these, [zomry1](https://ctftime.org/writeup/13273) also packs the write-up full of memes, so that's fun. [Two](https://ctftime.org/writeup/11748) additional write-ups use a bit-flipping tool in `BurpSuite Intruder` instead of native Python to do the bit flipping. Out of the two, I liked Dvd848's write-up, which walks the reader through the use of the tool and shows that with that approach the cookie can be brute-forced: it's not necessary to know which bit to flip.
