...
Dvd848 patches the file with `dd`. [tcode2k16](https://tcode2k16.github.io/blog/posts/picoctf-2018-writeup/forensics/#ext-super-magic) uses a Python script to do the patching. shiltemann uses the `debugfs` utility to determine that the magic number was corrupted, then mounts the file system instead of using `testdisk` to extract the files. This was a great example of a problem with multiple solutions, and I learned a bit from the write-ups.
