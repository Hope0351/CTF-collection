...
The four write-ups on CTFTime all use the same general approach. I learned some useful programming techniques from some of these write-ups. First, Dvd848 makes use of the `ror` and `rol` functions in Pwnlib, and also directly pastes data into a byte array (I did not realize that was possible). [Nine Inch Nulls](https://ctftime.org/writeup/11965) and Mithreindeir provide solutions in C.
