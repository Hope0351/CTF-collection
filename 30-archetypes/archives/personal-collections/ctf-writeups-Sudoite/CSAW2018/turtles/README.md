...
[Note: As this is a PDF of a Markdown document, some text may be cut off in the code sections. Please direct your browser to tf-writeups/tree/master/CSAW2018/turtles/turtles.md for the original write-up.]
...
I also needed to [reinstall](../../../../../../40-tooling/archives/pwn-tools/pwndbg) `pwndbg` from source. It turns out that the installation script works for Ubuntu 18.04 even though it's apparently not yet been tested. That gives me back my familiar `pwngdb` prompt in `gdb`.
...
But better yet, here's the source code:
...
Here's the source code for Objective C. Much of their code is written directly in assembly as an optimization -- it's quite fascinating. I suspect I'll have to understand how pointers are being followed in `objc_msg_lookup` so I can do a redirection, but for now I'll keep going.
...
Here's how a message is looked up:
...
`classForObject` is really basic and just returns a pointer to that `turtle` class. I care about `objc_dtable_lookup` which turns out to be defined as a `SparseArrayLookup`:
...
Note the commented-out sections: the `NSAutoreleasePool` is apparently for garbage collection. Apparently the `Turtle` program is supposed to complain around the time when I make a call to access the thread's `NSAutoreleasePool`. This happens when I try to log the turtle's spoken message in the turtle's `Say:` method: `NSLog` gets called, then `NSLogv` gets called, then some low-level code gets the path to the `GNUStep` configuration file during `GNUStepConfig` which can be seen here. Somewhere in that code a call to access the current `NSThread` object is made with `GSCurrentThread`. And there's a message made to complain, which sets off a whole new call to `NSLog` ("autorelease called without pool for object (%p) of class %@ in thread %@"), with appropriate arguments to that format string.
...
Stepping through, actually the problem is in a call to `malloc.c` from `NSZoneMalloc`. So, I updated my copy of `AngelHeap` to take a look. And wow! I wasn't prepared for just how much is sitting on the heap due to all these messages getting passed around in Objective-C:
