...
Some research later, we find a very handy tool that would allow us to recover the private key very easily called [RsaCtfTool](../../../../../40-tooling/archives/crypto-tools/RsaCtfTool). Despite the name, this tool can be very handy to quickly check cryptographic issues. First, we can get the public key in a .pem format from the JWT itself because we have the `n` and the `e` there already:  
...
```
We save the public key and private key in separate files and then we can use jwt_tool to tamper with the JWT and change the role to administrator.  
