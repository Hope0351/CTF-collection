...
Further googling allows us to find this POC. This one also has too many characters but in our context, we should be able to trim it down enough to be able to test and see if they are using a vulnerable ReportLab library:  
...
One such option would be to use Havoc C2 to generate a Windows Shellcode, obfuscate it using Harriet from Home-Grown-Read-Team to create an .exe payload.  
...
One such good POC is the very first result on Google:  
...
Now we need to find a way to use these credentials. Since we are still working from this reverse shell, we can use my script to upload a plugin for a reverse shell. Users can write their own, find mine, or forward the port. Uploading a .jar file to get a reverse shell is a well known technique going as far back as 2015 or so, there's even a metasploit module for it.
...
The powershell command is a Base64 encoded reverse shell generated with this powershell-obfuscator to bypass AV:
...
We can do some googling to find how to decrypt this Openfire password and find a helpful github page:;
