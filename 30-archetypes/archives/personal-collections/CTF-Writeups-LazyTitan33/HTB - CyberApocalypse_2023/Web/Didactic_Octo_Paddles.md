...
For the purposes of tampering with JWTs, I prefer to use the `jwt_tool` found here: wt_tool
...
According to the admin panel in the source code, the usernames use jsrender to render them however there is no sanitization in place. The user could be anything, including a SSTI payload which we can find here: acktricks/blob/master/pentesting-web/ssti-server-side-template-injection/README.md
