...
finally released. Katana is available at [https://github.com/JohnHammond/katana](../../../../40-tooling/archives/misc-tools/katana).** Pull-requests and contributions
are welcome!
...
* Java RMI
	...
	When testing this, responses are _known to come back with an error or exception_. Your code MAY VERY WELL still be executing. Try and run commands that include a callback. And _use Python_ to live off the land and try avoid special characters, like `|` pipes! ysoserial is a good tool for deserializing Java code to take advantage of this vulnerability.
...
* Other stego tools:
	...
	[https://github.com/DominicBreuker/stego-toolkit](../../../../40-tooling/archives/forensics-tools/stego-toolkit)
...
* [`jsteg`][jsteg]
    ...
    Another command-line tool to use against JPEG images. steg Handy for Hackerrank Codefest CTF 2018.
...
* Python random module cracker/predictor
	...
	ython-random-module-cracker... helps attack the Mersenne Twister used in Python's random module.
...
Some code for this attack can be found here
...
* [LC4]
	...
	C4/blob/master/documentation.md
...
* Magic Hashes
	...
	A common vulnerability in [PHP] that fakes hash "collisions..." where the `==` operator falls short in [PHP] type comparison, thinking everything that follows `0e` is considered scientific notation (and therefore 0). More valuable info can be found here: ashes, but below are the most common breaks.
...
* `/.git/`
	...
	A classic CTF challenge is to leave a `git` repository live and available on a website. You can see this with `nmap -A` (or whatever specific script catches it) and just by trying to view that specific folder, `/.git/`. A good command-line tool for this is `GitDumper.sh`, or just simply using [`wget`][wget].

	**Sometimes you might Bazaar or Mercurial or other distributed version control systems. You can use vcs-ripper for those!!**
...
* Bazaar `.bzr`
	...
	If you see a publically accessible `.bzr` directory, you can use `bzr branch http://site output-directory` to download it. Or, use this utility: vcs-ripper
...
* Mac / Macintosh / Apple Hidden Files `.DS_Store` ds_store_exp
	...
	On Mac computers, there is a hidden index file `.DS_Store`. You might be able to find it if you have an LFI vulnerability or something of the like. A good tool to track these down on a website is the DS_Store Exposer: s_store_exp.
...
* Wordsearches
	...
	Some CTFs have me solve wordsearchs as part of a challenge (TJCTF 2018). This code is super helpful: ord-search
...
[phpdc.phpr]:cache/blob/master/bin/phpdc.phpr
...
[StegCracker]: tegCracker
[StegSeek]: tegseek
[Base41]: ase41/blob/master/python/base41.py
[Base65535]: ase65536
[Easy Python Decompiler]: asy-Python-Decompiler-v1.3.2
[photorec]: https://www.cgsecurity.org/wiki/PhotoRec
[smbmap]: mbmap
[oletools]: letools
[impacket]: mpacket
[Responder]: esponder
[Responder.py]: esponder
...
[static-binaries]: tatic-binaries
