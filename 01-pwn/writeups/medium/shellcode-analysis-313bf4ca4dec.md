# :skull: Shellcode Analysis 313Bf4Ca4Dec

> **Original Source:** [Shellcode Analysis 313Bf4Ca4Dec](https://infosecwriteups.com/shellcode-analysis-313bf4ca4dec)
...
- Length — Because shellcode exploits a specific vulnerability in the memory, the sequence needs to be efficient as possible. It means the attacker has to suit its length to the buffer’s size, so all of the instructions will run in memory space.
...
- Goes to the PEB object.

- Passes through `PEB_LDR_DATA` object until it gets to `InMemoryOrderModuleList` linked list, which contains information about the processes' loaded modules. After this, the shellcode will hold the wanted module (usually `Kernel32.dll` is the third object).

- Finds Base-Address to the wanted DLL, using the `DllBase `field.

- Finds its Export Table.

- Locates the wanted function (like `GetProcAddress`).

- Gets the function’s address using the `GetProcAddress` function.

- Eventually, running the wanted function using its suitable parameters.
...
## Get Barak Aharoni’s stories in your inbox
...
*Originally published on [Medium](https://infosecwriteups.com/shellcode-analysis-313bf4ca4dec). All credit goes to the original author.*
