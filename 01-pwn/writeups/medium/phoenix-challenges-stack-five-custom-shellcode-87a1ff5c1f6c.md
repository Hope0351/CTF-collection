# :skull: Phoenix Challenges — Stack Five (Custom Shellcode)

> **Original Source:** [Phoenix Challenges — Stack Five (Custom Shellcode)](https://infosecwriteups.com/phoenix-challenges-stack-five-custom-shellcode-87a1ff5c1f6c)
...
This blog post is a bit different. Instead of a new challenge, it is an extension of the previous [Phoenix Stack Five writeup](https://medium.com/bugbountywriteup/phoenix-challenges-stack-five-pwntools-shellcode-5e5fe314d858). Those unfamiliar would need to review it before continuing.
...
We now start preparing the information needed to write our shellcode. Because the goal is to launch an `execve(filename = “/bin/sh”, …)`, we need to determine the `/bin/sh` string’s address. It will be passed as a pointer into the system call’s invocation. To do this, we load the Stack Five executable and check if `/bin/sh` is located anywhere:
...
We now take the *exploit.py* file from the previous [Phoenix Stack Five writeup](https://medium.com/bugbountywriteup/phoenix-challenges-stack-five-pwntools-shellcode-5e5fe314d858) and insert the following at its beginning:
...
We then delete the previous [Phoenix Stack Five writeup’s](https://medium.com/bugbountywriteup/phoenix-challenges-stack-five-pwntools-shellcode-5e5fe314d858) exploit.py file’s line for generating shellcode with Pwntools. The new shellcode is then fed in with:
...
*Originally published on [Medium](https://infosecwriteups.com/phoenix-challenges-stack-five-custom-shellcode-87a1ff5c1f6c). All credit goes to the original author.*
