...
### Portable release
The install script will automatically fetch and install the portable release from [GitHub releases](../../../../../40-tooling/archives/pwn-tools/pwndbg).
...
When installing with GDB, you may also download a package to install through your package manager of choice. Download the package from the [releases page](../../../../../40-tooling/archives/pwn-tools/pwndbg) and pick the appropriate download from the second table.
...
You can download a portable release on the [Pwndbg releases page](../../../../../40-tooling/archives/pwn-tools/pwndbg). There are separate releases for GDB and LLDB. Use the first table to pick the appropriate download for your system architecture. You can then unpack the archive with:
```{.bash .copy}
...
```{.bash .copy}
uv tool install git+wndbg[gdb,lldb]
```
...
uv tool install --python=$PY_VER  git+wndbg
```
...
In general, if you have the repository cloned you can run the same commands as in the above sections, but replacing `git+wndbg` with `.` (the current folder) and adding `--editable` so changes in the source are reflected in the installation.
