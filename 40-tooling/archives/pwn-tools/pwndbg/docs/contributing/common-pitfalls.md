...
See wndbg/pull/3492 for more info.
...
To fix this, ideally, you fix the source of the typing error and remove the comment. This is most often possible and will appease both `mypy` and `mypy --strict`. If this is not possible, you can often get around the issue by using `cast` as seen here: wndbg/blob/a35bf70366645b7bbd1359aee6e0caf0958aca87/pwndbg/integration/__init__.py#L182 . If that is also not possible due to the nature of the issue, tell us, and we will figure out what to do together.
