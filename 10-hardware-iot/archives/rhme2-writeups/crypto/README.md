...
Looking at the input string the first thing that comes to mind is a hash length extension attack. The provided hash values are 40 hex characters = 160 bits in length. This could be SHA-1. We can use standard tools like hashpump to perform the length extension attack on SHA-1. The only thing required to perform this attack is the length of the secret prefix. We can write a simple script to try all possible values for this length.
