...
For this challenge the unencrypted firmware file is provided so it can be analyzed. Like with any reverse engineering challenge it would be most interesting if static and dynamic analysis can be combined. For dynamic analysis it would be most conveniant to have an emulator that can interface with gdb. Luckily such an emulator exists in the form of simavr combined with the 'simduino' example which emulates an Arduino. 
